  ...
  plugins: [
    ...
    new HandlebarsPlugin({
      ...
      onBeforeSave: function (Handlebars, resultHtml, filename) {
        ...
        // Check if the current file is in the root of the dist folder
        const isRootFile = path.dirname(filePath) === distPath;
        
        let prefix;
        if (isRootFile) {
          prefix = './';
        } else {
          // Count the number of directory levels from the current file to the dist root
          const depth = relativePath.split(path.sep).length;
          prefix = '../'.repeat(depth);
        }

        // Remove any double slashes that might have been introduced
        return resultHtml.replace(/{{webRoot}}/g, prefix).replace(/([^:])\/\//g, '$1/');
      },
    }),
